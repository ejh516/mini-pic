...
bool LoadVolumeMesh(const string file_name, Volume &volume) { TRACE_ME;
    /*open file*/
...
bool LoadSurfaceMesh(const string file_name, Volume &volume, NodeType node_type) { TRACE_ME;
    /*open file*/
    ifstream in(file_name);
    if (!in.is_open()) {cerr<<"Failed to open "<<file_name<<endl; return false;}
...
/*saves particle data*/
void OutputParticles(vector<Particle> &particles) { TRACE_ME;
